...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 document.querySelectorAll('[data-os-resume]').forEach(b=>b.addEventListener('click',()=>{try{const m=JSON.parse(localStorage.getItem('inema.oswork-v2-es.meta')||'{}');if(m.lastModuleHref){const u=new URL(m.lastModuleHref,location.href);if(u.origin===location.origin){u.hash=(m.lastTopicAnchor||'').split('#')[1]||'';location.href=u.href;return}}}catch(e){}location.href='curso/trilha1/modulo-1-1.html'}));
 document.querySelectorAll('[data-task]').forEach(c=>{try{c.checked=localStorage.getItem('oswork.es.task.'+c.dataset.task)==='true'}catch(e){}c.addEventListener('change',()=>{try{localStorage.setItem('oswork.es.task.'+c.dataset.task,String(c.checked))}catch(e){}})});
 document.querySelectorAll('[data-copy-prev]').forEach(b=>b.addEventListener('click',async()=>{const text=b.previousElementSibling.innerText;try{await navigator.clipboard.writeText(text);b.textContent="Copiado"}catch(e){b.textContent="Selecciona el bloque y copia manualmente"}}));
});
