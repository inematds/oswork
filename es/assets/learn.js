...
(function (window, document) {
  ...
  function setReadButtonVisual(btn, isOn, sec) {
    ...
    var icon = btn.querySelector('.inema-read-icon');
  ...
  function buildSelectionPopover() {
    ...
    SWATCHES.forEach(function (c) {
      ...
      b.setAttribute('aria-label', c === 'doubt' ? "Marcar como duda" : ("Color de resaltado: " + {"todas": "todos", "yellow": "amarillo", "green": "verde", "blue": "azul", "pink": "rosa", "doubt": "duda"}[c]));
  ...
  function renderJourney(mountEl) {
    ...
    ['todas'].concat(SWATCHES).forEach(function (c) {
      var o = document.createElement('option'); o.value = c; o.textContent = {"todas": "todos", "yellow": "amarillo", "green": "verde", "blue": "azul", "pink": "rosa", "doubt": "duda"}[c]; selColor.appendChild(o);
    });
  ...
  function jumpTo(item) {
    ...
    if (target && target.scrollIntoView) {
      ...
      var match = String(item.id || '').match(/^modulo-(\d+-\d+)#(topico-\d+)$/);
      if (!match && item.blockId) {
        var blockMatch = String(item.blockId).match(/^m(\d+-\d+)-t(\d+)-/);
        if (blockMatch) match = [null, blockMatch[1], 'topico-' + blockMatch[2]];
      }
      if (match) navigateModule(match[1], match[2]);
    }
  }

  function navigateModule(moduleId, anchor) {
    var manifest = getManifest();
    if (!manifest) return false;
    var modules = [];
    manifest.tracks.forEach(function (track) { modules = modules.concat(track.modules || []); });
    var module = modules.find(function (entry) { return entry.id === moduleId; });
    if (!module || !module.href) return false;
    var destination = new URL(module.href, window.location.href);
  ...
  function resume() {
    ...
    if (match) return navigateModule(match[1], match[2]);
    return false;
  }
  ...
  function buildExportObject() {
    return Object.assign({}, storageGet('inema.' + S.courseId + '.extra', {}), {
      schemaVersion: SCHEMA_VERSION,
      courseId: S.courseId,
  ...
  function importJSON(text, opts) {
    ...
    if (parsed.courseId && parsed.courseId !== S.courseId) {
      res.errors.push("Este archivo pertenece a otro curso o idioma."); return res;
    }
    ...
    if (fields.some(function (key) { return parsed[key] !== undefined && !record(parsed[key]); }) ||
        Object.keys(parsed.read || {}).some(function (key) { return typeof parsed.read[key] !== 'boolean'; }) ||
        Object.keys(parsed.notes || {}).some(function (key) { return !Array.isArray(parsed.notes[key]) || parsed.notes[key].some(function (note) { return !record(note) || typeof note.id !== 'string' || typeof note.quote !== 'string'; }); })) {
      res.errors.push("Estructura de datos no válida."); return res;
    }
    var extras = storageGet('inema.' + S.courseId + '.extra', {});
    Object.keys(parsed).forEach(function (key) {
      if (fields.concat(['schemaVersion', 'courseId', 'exportedAt', '__proto__', 'constructor', 'prototype']).indexOf(key) === -1) extras[key] = parsed[key];
    });
    ...
    if (mode === 'replace') {
      ...
      storageSet('inema.' + S.courseId + '.extra', extras);
      res.ok = true;
      return res;
    }
